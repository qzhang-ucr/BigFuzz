...
public class JadTransUDF {
    String pathw ="customarray/src/edu/ucla/cs/bigfuzz/customarray/";

    private ArrayList<String> GeneratorUDF(int index, ArrayList<String> Source, Integer Operation){
        int s=0;
        ArrayList<String> UDFcode = new ArrayList<>();
        UDFcode.add("package edu.ucla.cs.bigfuzz.customarray;\n");
        UDFcode.add("public class mapValues10"+Operation+"{");
    ...
    public void FindUDF(ArrayList<String> Source, ArrayList<String> Operation) throws IOException {
        Pattern ForFind = Pattern.compile("\\.mapValues");
        Matcher m;
        ...
        for (String line : Source){
            ...
            if (m.find()){
                ArrayList<String> OperationNow = GeneratorUDF(LineNumber,Source,OperationNumber);
                FileWriter UDF = new FileWriter(pathw+"mapValues10"+OperationNumber+".java");
